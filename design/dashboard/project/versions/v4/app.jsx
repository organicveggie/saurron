...
function App() {
  ...
  // Synthetic running-cycle payload, used when the "Cycle running" tweak is
  // on. Real implementation would source this from polling `/v1/health`.
  const running = tweaks.cycleRunning ? {
  ...
  const wrap = (Layout, extra = {}) => (
    <div className="saurron-app" data-theme={tweaks.theme} data-accent={tweaks.accent}
      ...
      <Layout data={data} running={running} {...extra} />
    </div>
  );
  ...
  return (
    <>
      <DesignCanvas>
        ...
        >
          <DCArtboard id="timeline" label="Desktop · 1360×920"
            width={1360} height={920}>
            {wrap(LayoutTimeline)}
          </DCArtboard>
          <DCArtboard id="timeline-tablet" label="Tablet · 820×1180"
            width={820} height={1180}>
            {wrap(LayoutTimeline, { compact: true })}
          </DCArtboard>
        </DCSection>
      </DesignCanvas>
      ...
      <TweaksPanel title="Tweaks">
        <TweakSection label="State">
          <TweakToggle
            label="Cycle running"
            value={tweaks.cycleRunning}
            onChange={(v) => setTweak('cycleRunning', v)}
          />
        </TweakSection>
