...
function LayoutTimeline({ data, running = null, compact = false }) {
  ...
  return (
    <div style={{ flex: 1, display: 'flex', overflow: 'hidden', background: 'var(--bg)' }}>
      <NavDrawer active="dashboard" running={running} variant={compact ? 'rail' : 'standard'} />
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <TopAppBar
        title="Dashboard"
        subtitle="Update history & cycle activity"
        running={running}
        compact={compact}
      />

      <div style={{ flex: 1, overflow: 'auto', padding: compact ? '18px 20px 24px' : '24px 32px 32px' }}>
        {/* Summary cards — at tablet width, "Last cycle" spans both columns
            on row 1, with Updates/Failures sitting side-by-side on row 2. */}
        <div style={{
          display: 'grid',
          ...
          <StatCard
              ...
              chart={<Sparkline data={data.daily} w={compact ? 88 : 120} h={36} />}
              icon="upgrade" />
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }} data-comment-anchor="b950c4f9d2-div-67-9">
          ...
          <div style={{ flex: 1 }} />
          {/* Segmented "All / Updates / Failures" — desktop only. At tablet
              width we collapse this axis into the filter dropdowns. */}
          {!compact && (
            <div style={{
          ...
          <TimeWindowDropdown />
          <TriggerDropdown />
          {!compact && <button className="btn icon small" title="Refresh"><span className="ms">refresh</span></button>}
          {!compact && <button className="btn icon small" title="Export"><span className="ms">file_download</span></button>}
          {compact && (
          ...
          {groups.map((g, gi) =>
            <div key={g.label} style={{ marginBottom: gi === groups.length - 1 ? 0 : 8 }}>
              ...
              <div style={{ position: 'relative', paddingLeft: 0 }}>
                {/* vertical line — centered under the calendar icon (which
                    sits at x≈20 from the timeline rail's left edge) */}
                <div style={{
                  position: 'absolute',
                  left: 11.25, top: 0, bottom: 0, width: 1.5,
...
function TimelineEvent({ c, H, now, isExpanded, onToggle, compact = false }) {
  ...
  return (
    <div style={{ position: 'relative', paddingLeft: 48, paddingBottom: 8 }}>
      <div style={{
