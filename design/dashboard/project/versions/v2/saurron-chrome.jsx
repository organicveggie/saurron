...
// ============================================================
function NavDrawer({ active = 'dashboard', variant = 'standard' }) {
  ...
  if (variant === 'rail') {
    return (
      <nav style={{
        ...
        <div style={{ color: 'var(--primary)', marginBottom: 12 }}><SaurronLogo size={32} /></div>
        {items.map(it => (
          <button key={it.id} className="btn icon" title={it.label}
            style={{
              width: 56, height: 56, borderRadius: 16,
              background: it.id === active ? 'var(--secondary-container)' : 'transparent',
              color: it.id === active ? 'var(--on-secondary-container)' : 'var(--on-surface-variant)',
              flexDirection: 'column', gap: 2,
            }}>
            <span className={"ms" + (it.id === active ? ' fill' : '')} style={{ fontSize: 24 }}>{it.icon}</span>
            <span style={{ fontSize: 10, letterSpacing: '0.04em', fontWeight: 500 }}>{it.label.split(' ')[0]}</span>
          </button>
        ))}
        <div style={{ flex: 1 }} />
        <div className="chip success" style={{ height: 28, fontSize: 10, padding: '0 8px' }}>
          <span style={{ width: 6, height: 6, borderRadius: 999, background: 'currentColor', opacity: .9 }} />
          idle
        </div>
      </nav>
    );
    ...
    <nav style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 8px 18px' }}>
        ...
        <div>
          ...
          <div className="type-mono" style={{ color: 'var(--on-surface-muted)', fontSize: 11 }}>v0.7.0 · homelab-01</div>
        </div>
      </div>
...
// Top app bar
// ============================================================
function TopAppBar({ title = 'Dashboard', subtitle = null, density = 'normal' }) {
  ...
  return (
    <header style={{
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'baseline', gap: 14, minWidth: 0 }}>
        <div className="type-h1" style={{ fontWeight: 600 }}>{title}</div>
        {subtitle && <div className="type-body-sm" style={{ color: 'var(--on-surface-muted)' }}>{subtitle}</div>}
      </div>
      <div style={{
        position: 'relative',
