...
Some options have a single-letter short form as well, which look like `-f <value>`. Some options
take multiple arguments, which are always separated by commands:
...
The optional configuration file uses [TOML syntax](https://toml.io/en). The config file path
defaults to `/etc/saurron/config.toml`. You can override this via the `--config` CLI flag or the
`SAURRON_CONFIG` environment variable.
